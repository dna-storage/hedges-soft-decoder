...
do
	...
	echo "--------------------------------------------------------------" 
	sbatch --time "24:00:00"  --exclude="c21,c32,c34,c[58-59],c68"  -J squigulator -o ~/${out_file} -e ~/${err_file} -n 1  -p ${GPU}   --wrap "source ~/.bashrc && conda activate bonito_cuda && bonito basecaller dna_r9.4.1@v2 ~/squigulator/${fast5_path} --disable_koi --strand_pad GGCGACAGAAGAGTCAAGGTTC --hedges_bytes 204 ${byte_index} --hedges_params ~/hedges_options.json --disable_half --trellis base --processes 1 --lower_index ${lower_index} --upper_index ${upper_index}"
    done
done
